    ...
    <div className="emergency-notif-card">
      ...
      <div className="emergency-notif-info">
        <div className="emergency-notif-info-row">
          <Droplets size={15} />
          <strong style={{ color: "var(--brand-primary)" }}>{bg}</strong> blood
          required
        </div>
        <div className="emergency-notif-info-row">
...
export default function DonorDashboard() {
  ...
  const fetchAll = async () => {
    try {
      const [mRes, nRes, dRes] = await Promise.all([
        axios.get(
          "https://jeevanlink-production.up.railway.app/api/matches/donor",
        ),
        axios.get(
          "https://jeevanlink-production.up.railway.app/api/notifications",
        ),
        axios.get("https://jeevanlink-production.up.railway.app/api/donors/me"),
      ]);
  ...
  const handleRespond = async (matchId, status) => {
    try {
      await axios.put(
        `https://jeevanlink-production.up.railway.app/api/matches/${matchId}/respond`,
        { status },
      );
      toast.success(
        status === "ACCEPTED"
  ...
  const markRead = async (id) => {
    try {
      await axios.put(
        `https://jeevanlink-production.up.railway.app/api/notifications/${id}/read`,
      );
    } catch {}
  };
    ...
    <DashboardLayout title="Donor Dashboard">
      ...
      <div className="grid-4" style={{ marginBottom: "1.5rem" }}>
        <StatCard
          ...
          label="Eligibility"
          value={
            profile?.eligibility === "ELIGIBLE" ? "✓ Eligible" : "✗ Ineligible"
          }
          accentColor={
            profile?.eligibility === "ELIGIBLE" ? "#10b981" : "#ef4444"
          }
        />
        <StatCard
          icon={Droplets}
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div className="card-jl-body">
              ...
              >
                {[
                  {
                    label: "Blood Group",
                    value: <BloodGroupBadge group={profile?.bloodGroup} />,
                  },
                  {
                    label: "Eligibility",
                    value: (
                      <span
                        className={`badge-jl badge-${profile?.eligibility === "ELIGIBLE" ? "success" : "danger"}`}
                      >
                        {profile?.eligibility?.replace(/_/g, " ")}
                      </span>
                    ),
                  },
          ...
          <div className="card-jl">
            <div className="card-jl-header">
              <h3>Notifications</h3>
              {unread > 0 && (
                <span className="badge-jl badge-danger">{unread} new</span>
              )}
            </div>
            <div>
              {loading ? (
        ...
        <div className="card-jl">
          <div className="card-jl-header">
            ...
            {pending > 0 && (
              <span className="badge-jl badge-emergency">
                {pending} pending
              </span>
            )}
          </div>
          <div className="card-jl-body">
            {loading ? (
              ...
              <EmptyState
                ...
                description="You'll be notified here when a hospital nearby needs your blood type."
              />
            ) : (
              <div
