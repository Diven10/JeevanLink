...
function RequestForm({ emergency = false, onCreated }) {
  ...
  useEffect(
    () =>
      setForm((p) => ({ ...p, urgency: emergency ? "EMERGENCY" : "NORMAL" })),
    [emergency],
  );
  ...
  const submit = async (e) => {
    ...
    if (!Number(form.units) || Number(form.units) < 1)
      return toast.error("Enter at least 1 unit.");
    setSubmitting(true);
    try {
      const payload = {
        ...form,
  ...
  return (
    <div className="card-jl form-card">
      <div className="card-jl-header">
        <div>
          <h3 style={{ color: emergency ? "var(--danger)" : undefined }}>
            {emergency ? "🚨 Emergency Blood Request" : "New Blood Request"}
          </h3>
          <p className="muted" style={{ margin: "5px 0 0" }}>
            {emergency
              ? "This immediately starts donor and blood-bank matching."
              : "Create and track a hospital blood requirement."}
          </p>
        </div>
      ...
      <form className="card-jl-body" onSubmit={submit}>
        <div className="form-section">
          ...
          <label className="form-label-jl">
            Blood group required <span className="req">*</span>
          </label>
          <div className="blood-group-picker">
            {bloodGroups.map((bg) => (
              <button
                key={bg.value}
                type="button"
                className={`blood-group-option ${form.bloodGroup === bg.value ? "selected" : ""}`}
          ...
          <div className="grid-2">
            <div>
              <label className="form-label-jl">
                Units required <span className="req">*</span>
              </label>
              <input
                className="input-jl"
                type="number"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, requiredDate: e.target.value }))
                }
              />
            </div>
          </div>
            ...
            <div className="urgency-picker">
              {["NORMAL", "URGENT", "EMERGENCY"].map((u) => (
                <button
                  type="button"
                  key={u}
                  className={`urgency-option ${form.urgency === u ? `selected ${u.toLowerCase()}` : ""}`}
        ...
        <div className="form-section">
          ...
          <div className="grid-2">
            ...
            <div>
              <label className="form-label-jl">State</label>
              <input
                className="input-jl"
                value={form.state}
                onChange={(e) =>
        ...
        {form.urgency === "EMERGENCY" && (
          <div className="emergency-note">
            ...
            <span>
              Emergency requests trigger immediate compatibility matching and
              in-app notifications for potential donors and compatible verified
              blood banks.
            </span>
          </div>
        )}
        <button
          ...
          style={{
            ...
            background:
              form.urgency === "EMERGENCY" ? "var(--danger)" : undefined,
          }}
        >
          {submitting
            ? "Creating request…"
      ...
      <td>
        <div style={{ display: "flex", gap: 6 }}>
          ...
          {!["FULFILLED", "CANCELLED"].includes(request.status) && (
            <button
              className="btn-danger-jl btn-sm-jl"
    ...
    >
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        <StatCard
          icon={Droplets}
          label="Blood Group"
          value={request.bloodGroup?.replace("_POS", "+").replace("_NEG", "-")}
        />
        ...
        <div className="card-jl-body">
          {!request.matches?.length ? (
            <EmptyState
              ...
              description="Matching may still be running, or no compatible eligible donor has been found."
            />
          ) : (
            <div className="match-grid">
              {request.matches.map((m) => (
                <div className="match-card" key={m.id}>
                  <div>
                    <strong>{m.donor?.user?.name || "Potential donor"}</strong>
                    <div className="muted">
                      {m.donor?.bloodGroup
                        ?.replace("_POS", "+")
                        .replace("_NEG", "-")}{" "}
                      · {m.donor?.city || "Location not provided"}
                    </div>
                  </div>
...
export default function HospitalDashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/api/requests/hospital`);
      setRequests(res.data);
    } catch (err) {
      toast.error(
        err.response?.data?.error || "Failed to load hospital requests.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const cancel = async (id) => {
    ...
    try {
      await axios.put(`${API}/api/requests/${id}/cancel`);
  ...
  const pending = requests.filter((r) =>
    ["PENDING", "MATCHING", "MATCHED"].includes(r.status),
  ).length;
  ...
  if (path === "/hospital/emergency-request")
    return (
      <DashboardLayout
        title="Emergency Request"
        subtitle="Create an urgent request and start matching immediately."
      >
        <RequestForm
          emergency
          onCreated={() => {
  ...
  if (path === "/hospital/requests")
    return (
      <DashboardLayout
        ...
        subtitle="Create, monitor and manage all hospital blood requests."
      >
        <div className="page-toolbar">
          <div>
            <h2>Blood Requests</h2>
            <p className="muted">
              Every request is stored and tracked through its lifecycle.
            </p>
          </div>
          <div className="toolbar-actions">
            ...
            <button
              className="btn-primary-jl"
              onClick={() => navigate("/hospital/new-request")}
            >
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Create the first hospital blood request to start the workflow."
            action={
              <button
                className="btn-primary-jl"
                onClick={() => navigate("/hospital/new-request")}
  ...
  if (path === "/hospital/new-request")
    return (
      <DashboardLayout
        title="New Blood Request"
        subtitle="Create a standard blood requirement."
      >
        <RequestForm
          onCreated={() => {
            load();
    ...
    <DashboardLayout title="Hospital Dashboard">
      <div className="page-toolbar">
        <div>
          ...
          <p className="muted">
            Good to see you, <strong>{user?.name}</strong>. Monitor blood needs
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        <StatCard icon={Clock} label="Open Requests" value={pending} />
        <StatCard
          icon={AlertTriangle}
          label="Emergency"
          value={emergency}
      ...
      <div className="card-jl" style={{ marginBottom: 20 }}>
        <div className="card-jl-header">
        ...
        <div className="card-jl-body">
          <div className="emergency-banner">
            <div>
              <strong>Emergency workflow is ready.</strong>
              <p>
                Use the single <b>Emergency</b> action in the page header to
                start donor and blood-bank matching and notify compatible
                participants.
              </p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <AlertTriangle size={24} color="var(--danger)" />
              <span className="muted">Fast matching enabled</span>
            </div>
          </div>
        </div>
      </div>
      <div className="card-jl">
          ...
          requests.slice(0, 5).map((r) => (
            <div className="list-row" key={r.id}>
              <div>
                ...
                <div className="muted">
                  {r.units} unit(s) · {new Date(r.createdAt).toLocaleString()}
                </div>
              </div>
        ...
        {!loading && !requests.length && (
          <EmptyState
            ...
            description="Create a blood request to begin."
          />
        )}
      </div>
    </DashboardLayout>
