  ...
  const fetchAll = async () => {
    try {
      const [sRes, uRes] = await Promise.all([
        axios.get(
          "https://jeevanlink-production.up.railway.app/api/admin/stats",
        ),
        axios.get(
          "https://jeevanlink-production.up.railway.app/api/admin/users",
        ),
      ]);
      setStats(sRes.data);
      setUsers(uRes.data);
  ...
  return (
    <DashboardLayout title="Admin Dashboard">
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        >
          Administrator: <strong>{user?.name}</strong> — Full system access
        </p>
      </div>
        ...
        stats && (
          <div className="grid-4" style={{ marginBottom: "1.5rem" }}>
            ...
            <StatCard
              ...
              change={`of ${stats.totalRequests} total`}
              accentColor="#f59e0b"
            />
          </div>
        )
